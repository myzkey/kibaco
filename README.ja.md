...
アプリの起動コマンド、ローカル URL、リバースプロキシ、ログ、Docker サービス、ヘルスチェックを `~/.kibaco` 配下の workspace config に保存することで、プロジェクトディレクトリを汚さずに開発環境を管理します。
...
設定ファイル、proxyPort、Docker の状態、service 参照、project の cwd、service status、project URL、target の到達性を確認します。
