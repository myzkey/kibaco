...
import { ensureKibanDirs, projectPidPath, STATE_FILE } from "./paths.js";
import type { KibanState } from "./types.js";

const emptyState: KibanState = { projects: {} };

export async function readState(): Promise<KibanState> {
  await ensureKibanDirs();
  if (!(await fs.pathExists(STATE_FILE))) return emptyState;
  return { ...emptyState, ...(await fs.readJson(STATE_FILE)) };
}

export async function writeState(state: KibanState) {
  await ensureKibanDirs();
